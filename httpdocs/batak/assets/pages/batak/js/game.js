...
function playableCards(koz) {
	...
	//console.log(playableTpeInHand);
	//console.log(checkGroundSpades);
...
function playableCardses(koz,el,sira) {
    ...
    if(playableTpeInHand.length != 0) {
		if(checkGroundSpades.length === 0) {
			...
			if(playableTypeCardsBiggerThanGround.length != 0) {
				for(let j = 0; j < playableTypeCardsBiggerThanGround.length; j++) {
					for(let k = 0; k < computer2Cards.length; k++) {
						let cardMatched1 = cardRegex.exec(computer2Cards[k].style.background)[0].slice(6)
						let checkCard = playableTypeCardsBiggerThanGround[j].join("")
						if(cardMatched1 == checkCard) {
							computer2Cards[k].style.filter = 'none'
							computer2Cards[k].addEventListener('click', playerDropCard1)
						}
					}
				}
				}
