...
function contextekle() {	
	$.contextMenu( {
		...
		callback: function ( key, options ) {
			...
			var omesaj=options.items[ key ].omesaj
						...
						socket.emit( "userislemleri",
							{
								...
								islem: key,
								aciklama:result.value,
								omesaj:options.items[ key ].omesaj,
								sure:""
							} );
					}
				} );
			...
			if ( key == "salondan_banla" || key == "sistemden_banla" ) {
				...
				Swal.fire( {
					...
					cancelButtonText: 'Vazgeç',
					heightAuto: false,
					html: $( this ).find( ".person__info--text-name" ).text() + ' isimli kullanıcı ' + options.items[ key ].mesaj + '..<br> <textarea placeholder="Açıklama girmek ister misin?" class="swal2-textarea " style="width:95%!important;margin:5px;" id="exampleFormControlTextarea2" rows="3"></textarea>',
		...
		items: {
			"sustur": { name: "Sustur", icon: "fa-comment-slash", mesaj: "susturulacak",omesaj:"Susturuldunuz.." },
			"salondan_at": { name: "Salondan At", icon: "fa-right-from-bracket", mesaj: "Salondan atılacak",omesaj:"Salondan atıldınız.." },
			"sistemden_at": { name: "Sistemden At", icon: "fa-right-from-bracket", mesaj: "Sistem atılacak",omesaj:"Sistemden atıldınız.." },
			"salondan_banla": { name: "Salondan Banla", icon: "fa-ban", mesaj: "Salondan banlanacak",omesaj:"Salondan banlandınız.." },
			"sistemden_banla": { name: "Sistemden Banla", icon: "fa-ban", mesaj: "Sistemden banlanacak",omesaj:"Sistemden banlandınız.." },
			"quit": {
