								...
								<div class="card-body">									
									<div class="table-responsive">
										<table id="dataTableExample3" class="table table-sm">
											...
											<tbody>
												<?php 
													$datalar = $baglanti->query("SELECT hediye_gecmisi.*,
													(select adsoyad from oyuncular where id=hediye_gecmisi.gonderici) as gondericiadi,
													(select adsoyad from oyuncular where id=hediye_gecmisi.alici) as aliciadi,
													(select resim from hediyeler where id=hediye_gecmisi.hediye_id) as hediyeresmi,
													(select adi from hediyeler where id=hediye_gecmisi.hediye_id) as hediyeadi
													from hediye_gecmisi order by tarih desc");
													while ($data = $datalar->fetch(PDO::FETCH_ASSOC)) {
													?>
													<tr>
														<td><?php echo $data["gondericiadi"];?></td>
														<td><?php echo $data["aliciadi"];?></td> 
														<td><?php echo $data["hediyeadi"];?></td>
														<td><img src="../<?php echo $data["hediyeresmi"];?>" alt="<?php echo $data["hediyeadi"];?>"></td> 
