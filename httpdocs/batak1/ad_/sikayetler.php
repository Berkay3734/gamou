...
<html lang="en">
	<head>
		<title>Şikayet Bildirimleri</title>
		<?php				
			include 'head.php';
		?>
	</head>
								...
								<div class="card-body">									
									<div class="table-responsive">
										<table id="dataTableExample3" class="table table-sm">
											...
											<tbody>
												<?php 
													$datalar = $baglanti->query("SELECT * from sikayetler order by id desc");
													while ($data = $datalar->fetch(PDO::FETCH_ASSOC)) {
													?>
													<tr>
														<td><?php echo $data["sikayeteden"];?></td>
														<td><?php echo $data["sikayetedilen"];?></td> 
														<td><?php echo $data["konu"];?></td> 
