...
$(document).on( "click",".cikbuton", function() {
...
function esiminelinigoster(masa){
	...
	console.log("eşimin elini gamer");
...
function contextekle() {	
	$.contextMenu( {
		...
		callback: function ( key, options ) {
			...
			var contextusername=$( this ).find( ".person__info--text-name" ).text();
			if ( key == "salondan_at" || key == "sistemden_at" || key == "sustur" ) {
				Swal.fire( {
					...
					html:  contextusername+ ' isimli kullanıcı ' + options.items[ key ].mesaj + '..',
					heightAuto: false,
						...
						socket.emit( "userislemleri",
							{
								...
								islem: key,
								islem_adi:options.items[key].name,
