...
$(function() {   
	...
	socket.on("kozumbu", function(data){
		...
		$("#masakozbilgisi").find("img").attr("src","img/Cards/"+data.masa.kozkodu+"ZA.svg").attr('style','width: 40px');
	});
	...
	socket.on('userislemleri', function(data) {
		if(data.durum=="error"){
		...
		else if(data.islem=="sustur"){
			var bansebebi="";
			if(data.aciklama!=""){
				bansebebi="Susturulma Sebebi : <span class='fw-bold'>"+data.aciklama+"</span>";
			}
			console.log(hakkimda.hesap_detaylari.id,data.kullanici);
			if(hakkimda.hesap_detaylari.id==data.kullanici){
				Swal.fire({
					icon: "error",
					title: "Susturuldun",
					timer: 4000,
					...
					heightAuto: false,
					allowOutsideClick:false,
					html:"Yönetici tarafından susturuldun..<br>"+bansebebi
				})
			}
			var haberne="";
			haberne+='<div class="message">                                           '
			...
			haberne+='<span>Haberci:</span>                                           '
			haberne+='</span>                                                         '
			haberne+='<span class="word text-danger"> '+data.username+' kodlu kullanıcı susturuldu.. '+bansebebi+'</span>                               '
			haberne+='</div>		                                                  '
			$(".lobi-chat").append(haberne);
			sohbetlscrollindir();
		}
	});		
});		